...
function ResetPass() {
  ...
  const onSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!disableForm) {
      ...
      selectUserByEmail(emailEntered).then((users)=>{
        if(users?.length === 0){
          dispatch(hideLoading());
          return dispatch(showNotification("Nu există utilizator cu această adresă de mail!", NotificationType.ERROR));
        }
        const code = generateCode(6);
        axios.post("https://germanmaxserver.onrender.com/reset_mail",{ email: emailEntered, code }).then(async (response:AxiosResponse)=>{
           const {status} = response.data;

           if(status === "ok"){ 
            const currentDate = new Date();
            currentDate.setMinutes(currentDate.getMinutes() + 10);
            const expire_at = currentDate.toLocaleTimeString().replace("AM", "").replace("PM", "").trim();
            try{
              const isCodeSaved = await addExpiringCode(emailEntered,code,expire_at);
              console.log(isCodeSaved);
              dispatch(hideLoading());
              sessionStorage.setItem("emailToReset", emailEntered);
              navigate("/password_reset_code");
              dispatch(showNotification("Cod trimis!", NotificationType.SUCCESS));  
            }catch{
              serverError();
            }
           }else{
                ...
                <form onSubmit={onSubmit} className="forn">
                  ...
                  <div className="form__group">
                    <button className="btn btn--white" disabled={disableForm}>
                      Resetează&nbsp;<i className="fas fa-sync" />
                    </button>
                  </div>
                </form>
            </div>

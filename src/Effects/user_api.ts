...
export const selectUserByEmail = async function (emailSearch: string) {
  ...
  if (error) {
    console.error(error);
    throw new Error(
      `There is no user with the email address of ${emailSearch} !`,
    );
  }
  return user;
};
...
export const insertUser = async function (
  ...
  const { data, error } = await supabase
    .from("user")
    .insert([{ email, password: hashPassword, name, lastLevel }])
    .select();

  if (error) {
    console.error(error);
    throw new Error(
      `Could not do the insertion of the user!\nmail:${email}\npassword:${password}\nname:${name}\nlastLevel:${lastLevel}\n`,
    );
  }
...
export const addExpiringCode = async (
  email: string,
  code: string,
  expire_at: string,
) => {
